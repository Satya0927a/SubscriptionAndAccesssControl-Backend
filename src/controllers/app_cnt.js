...
approuter.post('/gpt', async (req, res, next) => {
  try {
    ...
    if (userSubs.plan.limits.MaxgenChatTokens != -1 && (userSubs.plan.limits.MaxgenChatTokens - userSubs.usage.genChatTokens) < 100) {
      return res.status(403).send({
        success: false,
...
approuter.post('/videogen', async (req, res, next) => {
  try {
    const { prompt } = req.body
    if (!prompt) {
      res.status(400).send({
    ...
    if (!userSubs.plan.access.genVideo) {
      return res.status(403).send({
        success: false,
        message: "This feature is not included in you subscription plan"
      })
    }
    if (userSubs.plan.limits.MaxgenVideo - userSubs.usage.genVideo == 0) { //will never hit 0 if limit is -1 that is unlimited
      return res.status(403).send({
        success: false,
        message: "You have exhausted your image gen limit"
      })
    }
    const gendata = "this is your generated videoo...."
    await subscriptionModel.findOneAndUpdate({userid:req.user.userid},{$inc:{"usage.genVideo":1}})
    res.send({
      success:true,
      message:"generated the video",
      gendata:gendata
    })
  } catch (error) {
    next(error)
